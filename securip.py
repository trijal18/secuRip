...
names=pd.read_csv(r"secuRIP\Indian-Male-Names.csv") #path to dataset

#get name form dataset
def get_name(): 
...
#genrate fake email coressponding to that name
def get_email(name):
...
for i in range(500):
    name = get_name()
    h=[
         name,genrate_usn(),get_email(name),get_number()
     ]
